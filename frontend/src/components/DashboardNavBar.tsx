...
export const DashboardNavBar = ({
  ...
  const isExpanded =
    ...
    const handleToggle = () => {
        ...
        else setInternalIsExpanded(next);
    };
    ...
    const primaryNavItems = navItems.filter((item) =>
        item.to === "/classes" || item.to === dashboardLink
    );
    ...
    return (
    <>
    <nav className="sidebar-shell-enter fixed left-0 top-0 h-dvh w-fit flex items-center select-none z-40">
        <div className={`relative flex justify-between items-center text-center flex-col h-dvh ${navWidthClasses} bg-[rgba(21,22,24,0.84)] border-r-1 border-[#353535] pt-5 pb-7 transition-all duration-200`}>
            <div className="flex justify-center items-center text-center flex-col pb-4">
          ...
          <img className="my-6" src="/web_images/dot.svg" alt="dot" />
          <div className="flex flex-col gap-3.5">
            {primaryNavItems.map((item) => (
              <div key={item.to} className="flex flex-col gap-3.5">
                <Link className="cursor-pointer" to={item.to} onClick={handleNavClick}>
                ...
                <img src="/web_images/dot.svg" className="w-[5px] h-[5px] my-5 self-center" alt="dot" />
            ...
            {secondaryNavItems.map((item) => (
              <div key={item.to} className="flex flex-col gap-3.5">
                ...
                {isNotesActionsMounted ? (
                  <div
                    className={`flex flex-col gap-3.5 transition-[opacity,transform] duration-[220ms] ease-out origin-top will-change-transform ${
